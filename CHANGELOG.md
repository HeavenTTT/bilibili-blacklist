...
- 面板「插件配置 → 网络与性能」新增 **`插件日志`** 下拉框（配置键 `logLevel`），三档：

  | 选项 | 取值 | 包含 |
  | --- | --- | --- |
  | 完全关闭（**默认**） | `off` | 除错误与告警外，插件不输出任何日志 |
  | 关键信息 | `info` | 分页初始化、脚本装载、自动连播跳转/停止等用户能感知的动作 |
  | 全部 | `verbose` | 再加上缓存刷新、观察器重连、每次网络拦截的过滤结果等过程性细节 |

- **错误与告警不受本开关影响**：所有 `console.error` / `console.warn` 调用点**保持原样、
  不经过日志模块**，因此永远输出。出问题时不会因为"日志关了"而什么都看不到。
- 实现：新增 `src/utils/logger.js`，提供 `blInfo()` / `blVerbose()` 两个分级入口
  （每次调用时读一遍级别，因此**改配置立即生效、无需刷新页面**）。
  全库 `console.log` 从 **43 处收敛到 1 处**（logger 自身的实现），其余全部改为分级调用：
  关键信息 18 处、过程细节 25 处。
- 详细档走 `console.debug`：它在 DevTools 默认的 "Info" 级别下不显示，
  想看时把 Console 级别切到 "Verbose" 即可，属于额外的一层过滤。
- 取值非法时（历史配置或手改）由 `storage.js` 回落到默认的 `"off"`。
- `src/utils/logger.js` 在 `build.config.json` 的模块顺序里紧跟 `storage.js`
  （分级函数是函数声明、有提升兜底，但排在配置之后语义更清楚）。

### 新增设定：首次进入播放页不执行自动连播处理
